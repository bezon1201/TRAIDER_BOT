...
- Уведомление админа при запуске: сообщение "Бот запущен. Версия 2.3".
- Команда `/start` отвечает: "Бот онлайн. Версия 2.3".
...
- **2.3** — устранён циклический импорт между `metrics` и `scheduler`, вынесены общие функции конфигурации в `scheduler_conf.py`; команды `/scheduler` и фоновой планировщик работают как в 2.2.
