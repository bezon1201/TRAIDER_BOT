...
- Уведомление админа при запуске: сообщение "Бот запущен. Версия 2.7".
- Команда `/start` отвечает: "Бот онлайн. Версия 2.7".
...
- **2.7** — шаг 5 чеклиста: запуск DCA-кампаний и первая сетка (`/dca start/status/stop`, файлы `<SYMBOL>_grid.json`, лог `grid_log.jsonl`) на базе 2.6.1 с исправленными массивами TA.
