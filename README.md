...
- **3.5** — логирование обработки команд `/trade mode` и ввода PIN.
- **3.4** — исправлена обработка команд `/trade mode` (ручный парс без фильтра Command).
- **3.3** — переписан main.py: подключён router `/trade mode`, версия бота берётся из BOT_VERSION.