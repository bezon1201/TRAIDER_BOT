# Trader Bot — версия 1.5

Телеграм-бот под деплой на Render, работающий через webhook.

## Что изменено в версии 1.5

- Ещё раз исправлена проверка имён файлов в `data.py` (корректная обработка символа `\`).
- Логика команд `/data`, `/now`, `/symbols`, `/help` остаётся прежней.
- Стартовое сообщение и ответ `/start` обновлены до версии 1.5.

## История версий

- **1.5** — фикс модуля `data.py`, устойчивый синтаксис проверки имён файлов.
...
- `BINANCE_API_KEY` — ключ Binance (не используется в 1.5)
- `BINANCE_API_SECRET` — секрет Binance (не используется в 1.5)
