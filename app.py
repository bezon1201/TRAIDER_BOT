...
async def _answer_callback(callback: dict) -> dict:
    ...
    if data.startswith("ORDERS_OPEN_L0:"):
        ...
        if week <= 0 or budget <= 0:
            msg = f"{symbol} {month}\nЦикл ещё не запущен (Wk{week}) или бюджет 0 — LIMIT 0 недоступен."
            await tg_send(chat_id, _code(msg))
            return {"ok": True}

        # режим рынка и автофлаг для LIMIT 0
        sdata = _load_symbol_data(symbol)
        market_mode = sdata.get("market_mode")
        raw_mode = market_mode.get("12h") if isinstance(market_mode, dict) else market_mode
        raw_mode_str = str(raw_mode or "").upper()
        ...
        perc = WEEKLY_PERCENT.get(mode_key, WEEKLY_PERCENT["RANGE"])
        p_l0 = int(perc.get("L0") or 0)
        if p_l0 <= 0:
            msg = f"{symbol} {month}\nДля уровня LIMIT 0 в режиме {mode_key} доля бюджета 0% — LIMIT 0 не используется."
            await tg_send(chat_id, _code(msg))
            return {"ok": True}

        # квота на LIMIT 0 по бюджету (одна неделя)
        quota = int(round(budget * p_l0 / 100.0))

        levels = get_pair_levels(symbol, month)
        lvl_state = (levels or {}).get("L0") or {}
        used = int(lvl_state.get("reserved") or 0) + int(lvl_state.get("spent") or 0)

        available = quota - used
        if available <= 0:
            msg = f"{symbol} {month}\nЛимит по LIMIT 0 уже исчерпан (доступно 0 USDC)."
            await tg_send(chat_id, _code(msg))
            return {"ok": True}

        if free <= 0:
            msg = f"{symbol} {month}\nСвободных средств по бюджету 0 USDC — открыть LIMIT 0 нельзя."
            await tg_send(chat_id, _code(msg))
            return {"ok": True}
        ...
        if free < available:
            msg = (
                f"{symbol} {month}\n"
                f"По уровню LIMIT 0 доступно {available} USDC, но свободно в бюджете только {free} USDC.\n"
                "Сначала освободите бюджет или уменьшите другие уровни."
            )
            await tg_send(chat_id, _code(msg))
            return {"ok": True}

        # автофлаг LIMIT 0 для текста
        flags = sdata.get("flags") or {}
        flag_l0 = flags.get("L0") or ""
        if flag_l0 == "🟢":
            flag_desc = "цена ниже / внизу коридора — можно брать по рынку"
        elif flag_l0 == "🟡":
            flag_desc = "можно открыть по рекомендациям"
        ...
        msg = (
            ...
            f"LIMIT 0 OPEN\n\n"
            f"Сумма: {available} USDC\n"
            f"Флаг: {flag_l0 or '-'} ({flag_desc})\n"
            f"Поставить виртуальный LIMIT 0-ордер на {available} USDC?"
        )

        kb = {
            "inline_keyboard": [
                [
                    {"text": "CONFIRM", "callback_data": f"ORDERS_OPEN_L0_CONFIRM:{symbol}:{available}"},
                    {"text": "↩️", "callback_data": f"ORDERS_BACK_MENU:{symbol}"},
                ]
            ]
        }
    ...
    if data.startswith("ORDERS_OPEN_L0_CONFIRM:"):
        ...
        # режим рынка и квота для LIMIT 0
        sdata = _load_symbol_data(symbol)
        market_mode = sdata.get("market_mode")
        raw_mode = market_mode.get("12h") if isinstance(market_mode, dict) else market_mode
        raw_mode_str = str(raw_mode or "").upper()
        ...
        # реальная сумма, которую можем зарезервировать сейчас
        actual = min(amount, available, free)
        if actual <= 0:
            msg = (
                f"{symbol} {month}\n"
                f"Не получилось зарезервировать средства на LIMIT 0.\n"
                f"Запрошено: {amount} USDC, доступно по уровню: {available} USDC, свободно в бюджете: {free} USDC."
            )
            await tg_send(chat_id, _code(msg))
            return {"ok": True}

        # обновляем состояние уровня LIMIT 0
        new_reserved = int(lvl_state.get("reserved") or 0) + actual
        levels["L0"] = {
            "reserved": new_reserved,
            "spent": int(lvl_state.get("spent") or 0),
