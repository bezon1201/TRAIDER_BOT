...
def _ceil(x: Decimal, step: Decimal) -> Decimal:
    if step <= 0: return x
    return x if (x % step) == 0 else ((x // step) + 1) * step
def _build_sym(snap: Dict) -> Dict:
    tf = snap.get("tf") or {}; t12 = tf.get("12h") or {}; t6 = tf.get("6h") or {}
    return {
...
def compute_oco_buy(snapshot: Dict, adaptive_params_func) -> Dict:
    ...
    tp = _floor(low, tick); trig = _ceil(high, tick); lim = trig - tick*_STOP_TICKS
    return {"TP Limit": float(tp), "SL Trigger": float(trig), "SL Limit": float(lim),
