...
def _fmt_budget_line(sym: str, val):
    try:
...
def handle_budget_command(text_norm: str) -> str:
    ...
    # Просто '/budget' — показать всё
    parts = text_norm.strip().split(maxsplit=1)
    if len(parts) == 1:
        if not budgets:
            return "Budget: empty"
        lines = ["Budget:"] + [_fmt_budget_line(sym, budgets[sym]) for sym in sorted(budgets.keys())]
        return "\n".join(lines)
    ...
    newv = round(max(0.0, newv), 2)
    budgets[sym] = newv
    budgets = _save_budget(budgets)
    return f"Budget set: {_fmt_budget_line(sym, budgets.get(sym, 0.0))}"
