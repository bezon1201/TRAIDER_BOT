...
TABLES={"UP":{"OCO":0.40,"L0":0.40,"L1":0.20,"L2":0.00,"L3":0.00},
...
def load_settings(sd:str)->dict:
    data_path=os.path.join(sd,SETTINGS)
    root_path=os.path.join(sd,"budget_long.json")
    cfg=_read_json(data_path,DEFAULT_SETTINGS)
    if cfg==DEFAULT_SETTINGS:
        legacy=_read_json(root_path,DEFAULT_SETTINGS)
        if legacy!=DEFAULT_SETTINGS:
            _atomic_write(data_path,legacy)  # migrate -> /data
            cfg=legacy
    return cfg
...
def load_state(sd:str)->dict:
    data_state=os.path.join(sd,STATE)
    root_state=os.path.join(sd,"budget_long_state.json")
    st=_read_json(data_state,DEFAULT_STATE)
    if st==DEFAULT_STATE:
        legacy=_read_json(root_state,DEFAULT_STATE)
        if legacy!=DEFAULT_STATE:
            _atomic_write(data_state,legacy)  # migrate -> /data
            st=legacy
...
def _localize(ts,tz): return ts+tz*3600
def _to_utc(ts,tz): return ts-tz*3600
def _iso(ts): 
...
def _core_set(sd:str)->set:
    ...
    for sym in pairs or []:
        ...
        try:
            p=os.path.join(sd,"data",f"{sym.upper()}.json")
            if not os.path.exists(p): p=os.path.join(sd,f"{sym.upper()}.json")
            j=_read_json(p,{})
            if (j.get("trade_mode") or j.get("mode") or "").upper()=="LONG":
                out.add(sym.upper())
        except Exception:
            pass
    return out

def _weekly_anchor(now_utc,tz):
    ...
    ws_local=sunday_10 if dt_local>=sunday_10 else sunday_10-datetime.timedelta(days=7)
    next_local=ws_local+datetime.timedelta(days=7)
    return _to_utc(ws_local.timestamp(),tz), _to_utc(next_local.timestamp(),tz)

def _month_anchors(now_utc,tz):
    dt_local=datetime.datetime.utcfromtimestamp(_localize(now_utc,tz))
    ms_local=dt_local.replace(day=1,hour=0,minute=0,second=0,microsecond=0)
    nm_local=ms_local.replace(year=ms_local.year+1,month=1) if ms_local.month==12 else ms_local.replace(month=ms_local.month+1)
...
def init_if_needed(sd:str,now_ts=None):
    import time as _t
    now_utc=now_ts or _t.time()
...
def weekly_tick(sd:str,market_modes:Dict[str,str])->dict:
    ...
    st["week_start_utc"],st["next_week_start_utc"],st["last_week_roll_utc"]=_iso(ws_utc),_iso(nxt_utc),_iso(now_utc)
...
def month_end_tick(sd:str)->dict:
    ...
    init_if_needed(sd)
    st=load_state(sd); cfg=load_settings(sd)
    now_utc=_t.time(); ms_utc,me_utc=_month_anchors(now_utc,int(cfg.get("tz_offset_hours",0) or 0))
    st["month_start_utc"],st["month_end_utc"],st["last_month_roll_utc"]=_iso(ms_utc),_iso(me_utc),_iso(now_utc)
    actions={}
